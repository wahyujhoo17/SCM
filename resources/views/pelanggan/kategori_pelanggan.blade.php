...
@section('konten')
        ...
        <tbody>
            @foreach ($data as $kategori)
                <tr id="tr_{{ $kategori->id }}">
                    <td>{{ $kategori->id }}</td>
                    <td>{{ $kategori->nama }}</td>
                    <td>{{ $kategori->nominal_diskon }}</td>
                    <td>
                        <div class="d-grid gap-2 d-md-block">
                            <button data-toggle="modal" id="edit" data-target=".UmodalKategori" class="btn btn-primary"
                                type="button">Edit</button>

                            <form method="POST" action="{{ route('kategori_pelanggan.destroy', $kategori->id) }}">
                                @csrf
    ...
    {{-- Modal Tambah Data --}}
    <form method="POST" action="{{ route('kategori_pelanggan.store') }}">
        @csrf
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        {{-- Diskon --}}
                        <div class="item form-group">
                            <label class="col-form-label col-md-3 col-sm-3 label-align" for="diskon">Diskon % <span
                                    class="required">*</span>
                            </label>
                            <div class="col-md-9 col-sm-9 ">
                                <input type="number" id="diskon" name="diskon" required="required"
                                    class="form-control ">
                            </div>
                        </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                                ...
                                                <div class="item form-group">
                                                    ...
                                                    <div class="col-md-9 col-sm-9 ">
                                                        <input type="number" id="Udiskon" name="Udiskon" required="required"
                                                        class="form-control ">
                                                    </div>
                                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            table.on('click', '#edit', function() {
                ...
                $('#Unama').val(data[1]);
                $('#Udiskon').val(data[2]);
                $('#editform').attr('action', 'kategori_pelanggan/' + data[0]);
            });
        });
