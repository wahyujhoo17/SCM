...
{{-- Kategori --}}
<div class="item form-group">
    <label class="col-form-label col-md-3 col-sm-3 label-align" for="Ujabatan">Jabatan <span class="required">*</span>
    </label>
    <div class="col-md-6 col-sm-6 ">
...
@if ($pegawai->outlet != '[]')
    <div class="item form-group">
        <label class="col-form-label col-md-3 col-sm-3 label-align" for="Ulokasi">lokasi <span
                class="required">*</span>
        </label>
        <div class="col-md-6 col-sm-6 ">
            <select class="form-control py-4" name="Ulokasi" id="Ulokasi" style="width: 100%;">
                @foreach ($outlet as $ou)
                    {{ $selected = '' }}
                    @if ($ou->id == $pegawai->outlet[0]->id)
                    {{ $selected = 'selected' }}
                    @endif
                    <option value="{{ $ou->id }}"{{ $selected }}>{{ $ou->nama }}</option>
                @endforeach
            </select>
        </div>
    </div>
